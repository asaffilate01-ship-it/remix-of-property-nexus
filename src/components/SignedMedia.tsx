...
export function SignedMedia({ path, kind, className }: { path: string; kind: "photo" | "video"; className?: string }) {
  const [url, setUrl] = useState<string | null>(null);
  const [err, setErr] = useState(false);
  const sign = useServerFn(signMediaUrl);
  useEffect(() => {
    let alive = true;
    sign({ data: { path, expires: 600 } })
      .then((r) => { if (alive) setUrl(r.url); })
      .catch(() => { if (alive) setErr(true); });
    return () => { alive = false; };
  }, [path]);
  if (err) return <div className={`flex items-center justify-center bg-muted ${className || ""}`}><ImageOff className="h-6 w-6 opacity-50" /></div>;
