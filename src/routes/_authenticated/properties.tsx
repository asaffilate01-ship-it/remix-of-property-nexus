...
import {
  TenantComplianceEditor,
  type TenantComplianceMap,
} from "@/components/tenancy/TenantComplianceEditor";
...
type Property = {
  ...
  listing_purpose: "rent" | "sale" | "both" | "short_let";
  notes: string | null;
...
const emptyProp = {
  ...
  hmo_licence_number: "",
  hmo_licence_expires: "",
  listing_purpose: "rent" as Property["listing_purpose"],
  notes: "",
  features: [] as string[],
  nightly_rate: "",
  min_stay_nights: "",
...
const emptyTenancy = {
  ...
  end_date: "",
  rent_amount: "",
  rent_frequency: "monthly" as "weekly" | "monthly",
  deposit: "",
  status: "draft",
  bio: {} as TenantBio,
...
function PropertiesPage() {
  ...
  const listProperty = async (p: Property) => {
    ...
    const listing_type = (
      p.listing_purpose === "short_let" ? "rent" : p.is_hmo ? "room" : purpose
    ) as "sale" | "rent" | "room";
  ...
  const filtered = useMemo(
    () =>
      rows.filter((p) => {
        ...
        if (q) {
          const t =
            `${p.title} ${p.address ?? ""} ${p.city ?? ""} ${p.postcode ?? ""}`.toLowerCase();
  ...
  const startEdit = (p: Property) => {
    ...
    const coverIdx = Math.max(
      0,
      photos.findIndex((x) => x.url === p.cover_image),
    );
    setForm({
      id: p.id,
      title: p.title,
      ...
      is_hmo: p.is_hmo,
      hmo_licence_number: p.hmo_licence_number ?? "",
      hmo_licence_expires: p.hmo_licence_expires ?? "",
      ...
      compliance: (p.compliance && typeof p.compliance === "object"
        ? p.compliance
        : {}) as ComplianceMap,
  ...
  const save = async () => {
    ...
    const coverUrl =
      form.photos[form.cover_index]?.url ?? form.photos[0]?.url ?? form.cover_image ?? null;
    const payload: any = {
      ...
      nightly_rate:
        form.listing_purpose === "short_let" && form.nightly_rate
          ? Number(form.nightly_rate)
          : null,
      min_stay_nights:
        form.listing_purpose === "short_let" && form.min_stay_nights
          ? Number(form.min_stay_nights)
          : null,
      cleaning_fee:
        form.listing_purpose === "short_let" && form.cleaning_fee
    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex gap-2">
            <Button
              ...
              title="Add with just a title + postcode, fill rest later"
            >
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Select
          ...
          onValueChange={(v) => {
            setFilterPurpose(v);
            if (v !== "all" && v !== "short_let") setFilterHoliday(null);
            if (v === "short_let") setFilterHoliday(true);
          }}
        >
        ...
        <div className="flex gap-1.5">
          <Button
            size="sm"
            variant={filterHoliday === true ? "default" : "outline"}
            onClick={() => setFilterHoliday(filterHoliday === true ? null : true)}
            className="gap-1"
          >
          ...
          <Button
            size="sm"
            variant={filterHoliday === false ? "default" : "outline"}
            onClick={() => setFilterHoliday(filterHoliday === false ? null : false)}
          >
            Long-let / Sale
          </Button>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p) => (
            <Card
              key={p.id}
              className="border-0 shadow-card hover:shadow-md transition-shadow cursor-pointer overflow-hidden"
              onClick={() => {
                setActive(p);
                setInitialTab("rooms");
              }}
            >
              <div className="aspect-[16/10] relative bg-muted">
                ...
                <div className="absolute top-2 right-2 flex gap-1 flex-wrap justify-end">
                  ...
                  <Badge
                    variant="secondary"
                    className="capitalize border-0 backdrop-blur bg-card/85"
                  >
                    {p.listing_purpose.replace("_", " ")}
                  </Badge>
                </div>
              </div>
              <CardContent className="p-5">
                ...
                <div className="text-sm text-muted-foreground truncate">
                  {[p.address, p.city, p.postcode].filter(Boolean).join(", ") || "No address"}
                </div>
                <div className="text-xs text-muted-foreground mt-2 flex gap-3 flex-wrap">
                  ...
                  {p.listing_purpose === "short_let" && p.nightly_rate && (
                    <span>£{p.nightly_rate}/night</span>
                  )}
                </div>
                ...
                <div className="mt-3 pt-3 border-t flex gap-2">
                  ...
                  <Button
                    ...
                    title="Create a marketplace listing from this property"
                  >
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm({ ...form, property_type: v, is_hmo: v === "hmo" ? true : form.is_hmo })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent className="max-h-72">
                  {(ptypes.length
                    ? ptypes.map((t) => ({ v: t.code, l: t.label }))
                    : PROPERTY_TYPES.map((t) => ({ v: t, l: t }))
                  ).map((t) => (
                    <SelectItem key={t.v} value={t.v} className="capitalize">
                      {t.l}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v: any) => setForm({ ...form, listing_purpose: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            {form.listing_purpose === "short_let" && (
              <>
                <div className="sm:col-span-2 pt-2 border-t text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  Short-let / Airbnb settings
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, min_stay_nights: e.target.value })}
                  />
                </div>
                <div>
            ...
            <div className="sm:col-span-2 flex items-center gap-3 pt-2 border-t">
              <Switch
                checked={form.is_hmo}
                onCheckedChange={(v) => setForm({ ...form, is_hmo: v })}
              />
            ...
            {form.is_hmo && (
              <>
                <div>
                  <Label>HMO licence #</Label>
                  <Input
                    value={form.hmo_licence_number}
                    onChange={(e) => setForm({ ...form, hmo_licence_number: e.target.value })}
                  />
                </div>
                <div>
                  <Label>HMO licence expires</Label>
                  <Input
                    type="date"
                    value={form.hmo_licence_expires}
                    onChange={(e) => setForm({ ...form, hmo_licence_expires: e.target.value })}
                  />
                </div>
                {!form.id && (
                  <div className="sm:col-span-2 text-xs text-muted-foreground bg-muted/40 rounded-md p-2 flex items-center gap-2">
                    <BedDouble className="h-3.5 w-3.5" /> After saving, you'll be taken to the Rooms
                    tab to add each lettable room.
                  </div>
                )}
              </>
            )}

            <div className="sm:col-span-2 pt-3 border-t text-xs font-medium text-muted-foreground uppercase tracking-wide">
              Marketing & specs
            </div>
            <div className="sm:col-span-2">
              ...
              <Textarea
                ...
                placeholder="Long-form property description used on listings…"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, price_qualifier: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              <Label>EPC</Label>
              <Select
                value={form.epc_rating || "none"}
                onValueChange={(v) => setForm({ ...form, epc_rating: v === "none" ? "" : v })}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.council_tax_band || "none"}
                onValueChange={(v) => setForm({ ...form, council_tax_band: v === "none" ? "" : v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="—" />
                </SelectTrigger>
            ...
            <div>
              <Label>Tenure</Label>
              <Select
                value={form.tenure || "none"}
                onValueChange={(v) => setForm({ ...form, tenure: v === "none" ? "" : v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="—" />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.furnished || "none"}
                onValueChange={(v) => setForm({ ...form, furnished: v === "none" ? "" : v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="—" />
                </SelectTrigger>
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              <Switch
                ...
                onCheckedChange={(v) => setForm({ ...form, bills_included: v })}
              />
            ...
            <div className="sm:col-span-2">
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Click the star to set the cover image. These photos flow through to any listing for
                this property.
              </p>
              <PhotoUploader
                photos={form.photos}
              ...
              <SheetHeader>
                <SheetTitle className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant="outline" className="capitalize">
                    {active.listing_purpose.replace("_", " ")}
                  </Badge>
                </SheetTitle>
                <div className="text-sm text-muted-foreground">
                  {[active.address, active.city, active.postcode].filter(Boolean).join(", ")}
                </div>
              </SheetHeader>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This removes rooms, tenancies and rent schedule. This cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction onClick={() => remove(active.id)}>
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
              ...
              <Tabs value={initialTab} onValueChange={setInitialTab} className="mt-6">
                <TabsList className="flex-wrap h-auto">
                  <TabsTrigger value="rooms">
                    <BedDouble className="h-3 w-3 mr-1" /> Rooms
                  </TabsTrigger>
                  <TabsTrigger value="tenancies">
                    <Users className="h-3 w-3 mr-1" /> Tenants
                  </TabsTrigger>
                  <TabsTrigger value="rent">
                    <PoundSterling className="h-3 w-3 mr-1" /> Rent
                  </TabsTrigger>
                ...
                <TabsContent value="rooms" className="mt-4">
                  <RoomsPanel propertyId={active.id} isHmo={active.is_hmo} />
                </TabsContent>
                <TabsContent value="tenancies" className="mt-4">
                  <TenanciesPanel propertyId={active.id} isHmo={active.is_hmo} />
                </TabsContent>
                <TabsContent value="rent" className="mt-4">
                  <RentPanel propertyId={active.id} />
                </TabsContent>
                <TabsContent value="schedule" className="mt-4">
                  <PropertyScheduleGantt propertyId={active.id} />
                </TabsContent>
                <TabsContent value="docs" className="mt-4">
                  <PropertyDocsPanel propertyId={active.id} />
                </TabsContent>
                <TabsContent value="compliance" className="mt-4">
                  <PropertyCompliancePanel propertyId={active.id} />
                </TabsContent>
              </Tabs>
            </>
          )}
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground">
          {rooms.length} rooms {isHmo && "• HMO"}
        </div>
        <Button size="sm" onClick={() => setEditing(emptyRoom)}>
          ...
          <CardContent className="p-3 flex items-center justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">
                {r.rent_pcm ? `£${Number(r.rent_pcm).toLocaleString()} pcm` : "no rent set"}{" "}
                {r.en_suite && "• en-suite"} {r.bills_included && "• bills inc."}
              </div>
            </div>
      ...
      {rooms.length === 0 && (
        <div className="text-sm text-muted-foreground text-center py-6 border border-dashed rounded-md">
          No rooms yet{isHmo && " — HMOs typically need one room per let"}
        </div>
      )}
        ...
        <DialogContent>
          ...
          {editing && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className={isHmo ? "" : "col-span-2"}>
                <Label>Name *</Label>
                <Input
                  value={editing.name}
                  onChange={(e) => setEditing({ ...editing, name: e.target.value })}
                  placeholder={isHmo ? "Master bedroom" : "Whole property"}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editing.rent_pcm}
                  onChange={(e) => setEditing({ ...editing, rent_pcm: e.target.value })}
                />
              </div>
              <div>
                <Label>Status</Label>
                ...
                >
                  ...
                  <SelectContent>
                    {["vacant", "reserved", "let", "off_market"].map((s) => (
              ...
              <div className="flex items-center gap-2 pt-6">
                <Switch
                  checked={editing.en_suite}
                  onCheckedChange={(v) => setEditing({ ...editing, en_suite: v })}
                />
                <Label>En-suite</Label>
              </div>
              <div className="flex items-center gap-2 pt-6">
                <Switch
                  checked={editing.bills_included}
                  onCheckedChange={(v) => setEditing({ ...editing, bills_included: v })}
                />
                <Label>Bills inc.</Label>
              </div>
            </div>
          )}
          <DialogFooter>
...
function TenanciesPanel({ propertyId, isHmo }: { propertyId: string; isHmo: boolean }) {
  ...
  const [rooms, setRooms] = useState<Room[]>([]);
  const [editing, setEditing] = useState<(typeof emptyTenancy & { tenant_id?: string }) | null>(
    null,
  );
  ...
  const [coTenants, setCoTenants] = useState<
    Record<string, { id: string; tenant_id: string; full_name: string; email: string | null }[]>
  >({});
  ...
  const load = useCallback(async () => {
    ...
    if (ids.length) {
      ...
      for (const l of (links as any[]) ?? []) {
        (map[l.tenancy_id] ||= []).push({
          id: l.id,
          tenant_id: l.tenant_id,
          full_name: l.tenants?.full_name ?? "Tenant",
          email: l.tenants?.email ?? null,
        });
      }
      setCoTenants(map);
    } else setCoTenants({});
  ...
  const addCoTenant = async (tenancyId: string) => {
    ...
    const { error } = await supabase
      .from("tenancy_tenants")
      .insert({ tenancy_id: tenancyId, tenant_id: tenantId });
    if (error) return toast.error(error.message);
    toast.success("Co-tenant added");
    setAddCoOpen(false);
    setAddCoPick("");
    setAddCoNew({ full_name: "", email: "", phone: "" });
    load();
  };
  ...
  const save = async () => {
    if (!editing || !editing.tenant_name.trim() || !editing.start_date || !editing.rent_amount)
      return toast.error("Name, start date and rent required");
    if (isHmo && rooms.length > 0 && !editing.room_id)
      return toast.error("Pick a room for this HMO tenant");
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <Button size="sm" onClick={() => setEditing(emptyTenancy)}>
          <Plus className="h-3 w-3 mr-1" /> Add tenant
        </Button>
      </div>
      {tenancies.map((t) => (
        <Card key={t.id}>
          <CardContent className="p-3 space-y-1">
            ...
            <div className="text-xs text-muted-foreground">
              £{Number(t.rent_amount).toLocaleString()} {t.rent_frequency} • {t.start_date}
              {t.end_date ? ` → ${t.end_date}` : ""}{" "}
              {t.room_id && `• ${rooms.find((r) => r.id === t.room_id)?.name ?? ""}`}
            </div>
            <div className="flex flex-wrap items-center gap-1 pt-1">
              ...
              <Button
                ...
                onClick={() => {
                  setAddCoOpen(true);
                  setEditing({ ...emptyTenancy, id: t.id });
                }}
              >
      ...
      {tenancies.length === 0 && (
        <div className="text-sm text-muted-foreground text-center py-6 border border-dashed rounded-md">
          No tenants yet — click "Add tenant" above
        </div>
      )}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="col-span-2">
                ...
                <Input
                  value={addCoNew.full_name}
                  onChange={(e) => setAddCoNew({ ...addCoNew, full_name: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  type="email"
                  value={addCoNew.email}
                  onChange={(e) => setAddCoNew({ ...addCoNew, email: e.target.value })}
                />
              </div>
              <div>
                <Label className="text-xs">Phone</Label>
                <Input
                  value={addCoNew.phone}
                  onChange={(e) => setAddCoNew({ ...addCoNew, phone: e.target.value })}
                />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button onClick={() => editing?.id && addCoTenant(editing.id)}>Add</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing?.id ? "Edit tenant" : "Add tenant"}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-4">
                ...
                <div className="rounded-md border bg-muted/30 p-3">
                  ...
                  <Select value={editing.tenant_id ?? ""} onValueChange={pickExisting}>
                    <SelectTrigger className="mt-1.5">
                      <SelectValue placeholder="Choose from tenant directory…" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    type="email"
                    value={editing.tenant_email}
                    onChange={(e) => setEditing({ ...editing, tenant_email: e.target.value })}
                  />
                </div>
                <div>
                  <Label>Phone</Label>
                  <Input
                    value={editing.tenant_phone}
                    onChange={(e) => setEditing({ ...editing, tenant_phone: e.target.value })}
                  />
                </div>
                {rooms.length > 0 && (
                  <div className="col-span-2">
                    <Label>Room {isHmo && "*"}</Label>
                    <Select
                      value={editing.room_id}
                      onValueChange={(v) => setEditing({ ...editing, room_id: v })}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={isHmo ? "Pick a room" : "Whole property"} />
                      </SelectTrigger>
                      <SelectContent>
                        {rooms.map((r) => (
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={editing.rent_amount}
                    onChange={(e) => setEditing({ ...editing, rent_amount: e.target.value })}
                  />
                </div>
                <div>
                  <Label>Frequency</Label>
                  <Select
                    value={editing.rent_frequency}
                    onValueChange={(v: any) => setEditing({ ...editing, rent_frequency: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditing({ ...editing, deposit: e.target.value })}
                  />
                </div>
                <div>
                  <Label>Status</Label>
                  ...
                  >
                    ...
                    <SelectContent>
                      {["draft", "active", "notice", "ended"].map((s) => (
                        <SelectItem key={s} value={s} className="capitalize">
                          {s}
                        </SelectItem>
                      ))}
              ...
              />
              <TenantComplianceEditor
                value={editing.tenant_compliance}
                onChange={(tc) => setEditing({ ...editing, tenant_compliance: tc })}
              />
              {editing.id && (
                <TenantDocsMini
                  tenancyId={editing.id}
                  tenantName={editing.tenant_name || "tenant"}
                />
              )}
              {!editing.id && (
                <p className="text-xs text-muted-foreground border border-dashed rounded p-3">
                  Save the tenant first, then re-open to upload documents (Right-to-Rent, passport,
                  references, signed AST).
                </p>
              )}
            </div>
          )}
...
function RentPanel({ propertyId }: { propertyId: string }) {
  ...
  const markPaid = async (r: any) => {
    const { error } = await supabase
      .from("rent_schedule")
      .update({ status: "paid", paid_amount: r.amount, paid_at: new Date().toISOString() })
      .eq("id", r.id);
    if (error) return toast.error(error.message);
  ...
  if (!rows.length)
    return (
      <div className="text-sm text-muted-foreground text-center py-6 border border-dashed rounded-md">
        No rent schedule. Use the £ button on a tenancy to generate one.
      </div>
    );
          ...
          >
            <div className="min-w-0">
              <div className="font-medium truncate">
                {r.tenant_name} • £{Number(r.amount).toLocaleString()}
              </div>
              <div className="text-xs text-muted-foreground">
                Due {r.due_date}{" "}
