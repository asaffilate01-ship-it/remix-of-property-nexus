...
function DocumentsPage() {
  ...
  const { docs, properties, agencies, tenancies, profiles } = useMemo(
    () => ({
      docs: data?.documents ?? [],
  ...
  const landlords = useMemo(
    () => profiles.filter((profile) => profile.primary_role === "landlord"),
    [profiles],
  );
  const tenants = useMemo(
    () => profiles.filter((profile) => profile.primary_role === "tenant"),
    [profiles],
  );
  ...
  const scopeOptions = useMemo(() => {
    switch (form.scope) {
      case "property":
        return properties.map((p) => ({ value: p.id, label: p.title }));
      case "landlord":
        return landlords.map((c: { id: string; full_name: string | null }) => ({
          value: c.id,
          label: c.full_name ?? c.id,
        }));
      case "tenant":
        return tenants.map((c: { id: string; full_name: string | null }) => ({
          value: c.id,
          label: c.full_name ?? c.id,
        }));
      case "tenancy":
      ...
      case "agency":
        return agencies.map((a) => ({ value: a.id, label: a.name }));
    }
  ...
  const visible = docs.filter((d) => {
    ...
    if (q && !`${d.name} ${(d.tags || []).join(" ")}`.toLowerCase().includes(q.toLowerCase()))
      return false;
    return true;
  });
  ...
  function scopeLabel(d: (typeof docs)[number]) {
    ...
    if (d.scope === "landlord")
      return landlords.find((c: { id: string }) => c.id === d.landlord_user_id)?.full_name;
    if (d.scope === "tenant")
      return tenants.find((c: { id: string }) => c.id === d.tenant_user_id)?.full_name;
  ...
  const expiringSoon = docs.filter(
    (d) => d.expires_on && new Date(d.expires_on) < new Date(Date.now() + 90 * 86400000),
  ).length;
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Scoped per property, landlord, tenant, tenancy or agency. Encrypted & audit-logged.
          </p>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select
                    value={form.scope}
                    onValueChange={(v) => setForm({ ...form, scope: v as DocScope, scope_id: "" })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  value={form.scope_id}
                  onValueChange={(v) => setForm({ ...form, scope_id: v })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={`Pick a ${form.scope}`} />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold mt-1">{(totalSize / 1024 / 1024).toFixed(1)} MB</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className={`text-2xl font-bold mt-1 ${expiringSoon ? "text-warning" : ""}`}>
              {expiringSoon}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold mt-1">{docs.filter((d) => d.locked).length}</div>
          </CardContent>
        </Card>
      </div>
          ...
          {isLoading ? (
            ...
            <Card className="border-dashed">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Upload a file and attach it to a property, landlord, tenant, tenancy or your
                  agency.
                </p>
              </CardContent>
            </Card>
                ...
                <div className="divide-y">
                  {visible.map((d) => {
                    ...
                    return (
                      <div
                        key={d.id}
                        className="px-4 py-3 grid grid-cols-[auto_minmax(0,1fr)_auto] gap-3 items-center hover:bg-muted/30 transition-colors"
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1 mt-1 items-center">
                            <Badge variant="secondary" className="text-[10px] py-0">
                              <Icon className="h-3 w-3 mr-1" />
                              {SCOPE_META[d.scope as DocScope]?.label}: {scopeLabel(d) ?? "—"}
                            </Badge>
                        ...
                        <div className="flex gap-1 shrink-0">
                          <Button size="icon" variant="ghost" onClick={() => download(d.id)}>
