...
function expiryState(
  value: string | null,
  now = Date.now(),
): "expired" | "soon" | "valid" | "missing" {
  if (!value) return "missing";
  const expiry = new Date(value).getTime();
  if (expiry < now) return "expired";
  if (expiry - now < 30 * 86_400_000) return "soon";
  return "valid";
}
...
function CompliancePage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      expired: records.filter((record) => expiryState(record.expires_on, now) === "expired").length,
      soon: records.filter((record) => expiryState(record.expires_on, now) === "soon").length,
      valid: records.filter((record) => expiryState(record.expires_on, now) === "valid").length,
    };
  }, [records]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Compliance"
        description="EPC, gas, electrical, HMO and other statutory certificates."
        actions={
          <Button
            onClick={() => {
              setForm(empty);
      ...
      {loading ? (
        ...
        <Card className="border-dashed border-2 bg-transparent">
          <CardContent className="p-12 text-center text-muted-foreground">
            <ShieldCheck className="mx-auto h-10 w-10 mb-3 opacity-40" />
            <div>No compliance records yet.</div>
          </CardContent>
        </Card>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  const expiry = expiryState(r.expires_on);
                  const isExpired = expiry === "expired";
                  const isSoon = expiry === "soon";
                  return (
                    <tr key={r.id} className="border-b hover:bg-muted/30">
                      <td className="p-3">
                        <div className="font-medium">{rule?.label ?? r.type}</div>
                        {r.reference && (
                          <div className="text-xs text-muted-foreground">{r.reference}</div>
                        )}
                      </td>
                      <td className="p-3 text-xs text-muted-foreground">
                        {prop ? [prop.address, prop.city].filter(Boolean).join(", ") : "—"}
                      </td>
                      <td className="p-3 text-xs">
                        {r.issued_on ? new Date(r.issued_on).toLocaleDateString() : "—"}
                      </td>
                      <td className="p-3 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {["valid", "due_soon", "expired", "missing"].map((s) => (
            ...
            <div className="col-span-2">
              ...
              <Select
                ...
                onValueChange={(val) => setForm({ ...form, property_id: val })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select" />
                </SelectTrigger>
