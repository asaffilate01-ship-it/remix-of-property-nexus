...
function BranchesPage() {
  ...
  useEffect(() => { void load(); }, [fetchBranches]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Branches" description="Office locations under your agency." actions={
        <Button onClick={() => { setForm(empty); setOpen(true); }} disabled={!agencyId || !isOwner}><Plus className="mr-2 h-4 w-4" /> Add branch</Button>
      } />

      {!agencyId && !loading && (
        <Card className="border-dashed border-2 bg-transparent"><CardContent className="p-12 text-center text-muted-foreground">No agency is connected to this account. Set one up under Agency settings first.</CardContent></Card>
      )}

      {agencyId && !isOwner && !loading && (
        <Card className="border-0 bg-muted/40"><CardContent className="p-4 text-sm text-muted-foreground">Branch details are read-only. Only the agency owner can add, edit or remove branches because changes can affect billing.</CardContent></Card>
      )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          {!form.id && <p className="text-xs text-muted-foreground">Adding a branch updates your subscription quantity and may create a prorated Stripe charge.</p>}
