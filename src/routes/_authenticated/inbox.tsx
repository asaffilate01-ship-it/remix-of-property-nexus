...
function Inbox() {
  ...
  const send = async () => {
    ...
    const { error } = await supabase.from("messages").insert({ thread_id: active.id, sender_id: me, body: text });
  ...
  const createThread = async () => {
    ...
    if (newEmail) {
      const { data: p } = await supabase.from("profiles").select("id").ilike("full_name", `%${newEmail}%`).maybeSingle();
      participantId = p?.id ?? null;
    }
    const { data: agencies } = await supabase.from("agencies").select("id").eq("owner_id", me).limit(1);
    const { data: t, error } = await supabase.from("message_threads").insert({
      subject: newSubject, created_by: me, agency_id: agencies?.[0]?.id ?? null,
    }).select("*").single();
    if (error) return toast.error(error.message);
    if (participantId) await supabase.from("thread_participants").insert({ thread_id: t!.id, user_id: participantId });
    setNewOpen(false); setNewSubject(""); setNewEmail("");
    await loadThreads(); setActive(t);
  };
    ...
    <div className="h-[calc(100vh-8rem)] flex gap-4">
          ...
          <Dialog open={newOpen} onOpenChange={setNewOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div><Label>Subject</Label><Input value={newSubject} onChange={(e) => setNewSubject(e.target.value)} /></div>
                <div><Label>Add participant (name search, optional)</Label><Input value={newEmail} onChange={(e) => setNewEmail(e.target.value)} /></div>
              </div>
              <DialogFooter><Button onClick={createThread}>Create</Button></DialogFooter>
            </DialogContent>
          </Dialog>
        </CardHeader>
      ...
      <Card className="flex-1 flex flex-col">
        {!active ? (
          ...
          <>
            <CardHeader className="border-b"><CardTitle className="text-base">{active.subject}</CardTitle></CardHeader>
            <CardContent className="flex-1 overflow-y-auto space-y-2 py-4">
              {messages.map((m) => (
                <div key={m.id} className={`flex ${m.sender_id === me ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-md rounded-2xl px-3 py-2 text-sm ${m.sender_id === me ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                    {m.body}
            ...
            <div className="border-t p-3 flex gap-2">
              <Textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Type a message…" rows={2}
